...
class SillysCheatLoader(ctk.CTk):
    ...
    def load_config(self):
        if os.path.exists(self.config_file):
            ...
            try:
                self.discord_invite = self.cheats.get("__meta__", {}).get("discord_invite", "https://discord.gg/example")
            except Exception:
            ...
            self.cheats = {
                ...
                "GTA5": {"path": os.path.join(self.cheat_folder, "gta5.exe"), "status": "Online", "last_update": "0 Days Ago", "version": "1.2"},
                ...
                "MTA": {"path": os.path.join(self.cheat_folder, "mta.exe"), "status": "Online", "last_update": "0 Days Ago", "version": "1.2"}
            }
    ...
    def create_ui(self):
        ...
        for game in self.cheats.keys():
            btn = ctk.CTkButton(sidebar, text=game, fg_color="transparent", hover_color="#9932CC", anchor="w",
    ...
    def show_status_message(self, message, color="green"):
        if self.status_message:
            ...
            self.after(5000, lambda: self.status_message.configure(text=""))  # Hide after 5s
    ...
    def open_settings(self):
        ...
        def save_settings():
            try:
                ...
                settings.destroy()
                # Refresh UI
                self.select_game(self.selected_game) if self.selected_game else self.reset_to_home()
